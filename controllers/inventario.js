...
getInventarioID: async (req, res) => {
    ...
    postInventario: async (req, res) => {
        try {
        const {descripcion,valor,cantidad, estado, expirationDate} = req.body
        const codigo = await obtenerSiguienteCodigo();

        const inventario = new Inventario({codigo,descripcion,valor,cantidad, estado, expirationDate})
